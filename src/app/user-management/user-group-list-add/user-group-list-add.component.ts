...
@Component({selector: 'app-user-group-list-add', templateUrl: './user-group-list-add.component.html', styleUrls: ['./user-group-list-add.component.scss']})
export class UserGroupListAddComponent implements OnInit {
    ...
    goBack() {
        this.router.navigate(['/users/groups/' + this.userId]);
        return true;
    }
    ...
    async addRoles() {
        try {
            this.isLoadingResults = true;
            await this.userService.addRolesToUser(this.userId, this.selection.selected).toPromise();
            this.messageService.displayMessage('Added Roles');
            this.isLoadingResults = false;
            this.goBack();
        } catch (error) {
            this.messageService.displayMessage('Failed adding roles');
            this.isLoadingResults = false;
        }
    }
    ...
    async addRole(role: string) {
        try {
            ...
            const roles = [];
            roles.push(role);
            await this.userService.addRolesToUser(this.userId, roles).toPromise();
            this.messageService.displayMessage('Added role: ' + role);
            this.isLoadingResults = false;
            this.goBack();
        } catch (error) {
            this.messageService.displayMessage('Failed adding role: ' + role);
            this.isLoadingResults = false;
        }
    }
    // private exportData(data: Clients[]): object[] {
