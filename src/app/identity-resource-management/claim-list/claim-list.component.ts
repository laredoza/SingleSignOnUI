...
export class ClaimsListComponent implements OnInit {
  ...
  dataSource: DataSourceResponse<IdentityResourceClaim[]>;
  selection = new SelectionModel<IdentityResourceClaim>(true, []);
  ...
  constructor(
    ...
    this.dataSource = new DataSourceResponse<IdentityResourceClaim[]>();
    this.dataSource.data = new Array<IdentityResourceClaim>();
...
async loadData(
    all: boolean
  ): Promise<DataSourceResponse<IdentityResourceClaim[]>> {
    ...
    try {
      ...
      const result = await this.service
        .returnIdentityResourceClaims(this.dataSourceRequest, this.name)
        .toPromise();
  ...
  async export() {
    ...
    XLSX.writeFile(workBook, "IdentityResourceClaims.xlsx");
  }
  ...
  async removeClaims() {
    try {
      ...
      await await this.service
        .removeClaimsFromIdentityResource(this.selection.selected)
        .toPromise();
  ...
  async removeClaim(claim: IdentityResourceClaim) {
    try {
      this.isLoadingResults = true;
      var claims = new Array<IdentityResourceClaim>();
      claims.push(claim);
      await this.service
        .removeClaimsFromIdentityResource(claims)
        .toPromise();
