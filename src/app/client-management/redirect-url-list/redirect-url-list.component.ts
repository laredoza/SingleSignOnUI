...
@Component({
  selector: "app-redirect-url-list",
  templateUrl: "./redirect-url-list.component.html",
  styleUrls: ["./redirect-url-list.component.scss"]
})
export class RedirectUrlListComponent implements OnInit {
  ...
  dataSource: DataSourceResponse<ClientRedirectUri[]>;
  selection = new SelectionModel<ClientRedirectUri>(true, []);
  ...
  constructor(
    ...
    this.dataSourceRequest.sort.push(
      new DatasourceSort("RedirectUri", DataSourceSortDirection.Desc)
    );
    ...
    this.dataSource = new DataSourceResponse<ClientRedirectUri[]>();
    this.dataSource.data = new Array<ClientRedirectUri>();
  ...
  InitializeGridColumns() {
    ...
    this.displayedColumns.push("redirectUri");
    this.displayedColumns.push("remove");
  }

  async loadData(all:boolean) : Promise<DataSourceResponse<ClientRedirectUri[]>> {
        ...
        try {
            ...
            const result = await this.service.returnClientRedirectUris(this.dataSourceRequest, this.clientId).toPromise();
            ...
            const mappedData = [... result.data].map((row: ClientRedirectUri) => {
                return row;
            });
            this.isLoadingResults = false;
            return result;
        } catch (err) {
            this.messageService.displayMessage('Failed loading client redirect uri\'s');
  ...
  async onSearch(value: string): Promise<void> {
      ...
      if (value !== '')
      {
          this.dataSourceRequest.filter = new DataSourceRequestFilter("RedirectUri.ToLower()", DataSourceOperator.Contains,value == '' ? null : value, DataSourceLogic.Or)
      }
      else
      {
  ...
  async export()
  {
      ...
      XLSX.writeFile(workBook, 'ClientRedirectUris.xlsx');
  }
  ...
  async removeScopes() {
    try {
      this.isLoadingResults = true;
      await this.service
        .removeRedirectUrisFromClient(this.selection.selected)
        .toPromise();
      this.messageService.displayMessage("Removed RedirectUri's");
      this.isLoadingResults = false;
      this.refresh();
    } catch (error) {
      this.messageService.displayMessage("Failed removing RedirectUri's");
      this.isLoadingResults = false;
    }
  }

  async removeScope(redirectUri: ClientRedirectUri) {
    try {
      this.isLoadingResults = true;
      const redirectUris = new Array<ClientRedirectUri>();
      redirectUris.push(redirectUri);
      await this.service.removeRedirectUrisFromClient(redirectUris).toPromise();
      this.messageService.displayMessage("Removed scope: " + redirectUri.redirectUri);
      this.isLoadingResults = false;
      this.refresh();
    } catch (error) {
      this.messageService.displayMessage(
        "Failed removing RedirectUri: " + redirectUri.redirectUri
      );
      this.isLoadingResults = false;
    }
  }
