...
@Component({
  selector: "app-cors-url-list",
  templateUrl: "./allowed-cors-url-list.component.html",
  styleUrls: ["./allowed-cors-url-list.component.scss"]
})
export class AllowedCorsUriListComponent implements OnInit {
  ...
  dataSource: DataSourceResponse<ClientCorsUri[]>;
  selection = new SelectionModel<ClientCorsUri>(true, []);
  ...
  constructor(
    ...
    this.dataSourceRequest.sort.push(
      new DatasourceSort("Origin", DataSourceSortDirection.Desc)
    );
    ...
    this.dataSource = new DataSourceResponse<ClientCorsUri[]>();
    this.dataSource.data = new Array<ClientCorsUri>();
  ...
  async loadData(all:boolean) : Promise<DataSourceResponse<ClientCorsUri[]>> {
        ...
        try {
            ...
            const result = await this.service.returnCorsUris(this.dataSourceRequest, this.clientId).toPromise();
            ...
            const mappedData = [... result.data].map((row: ClientCorsUri) => {
                return row;
            });
            this.isLoadingResults = false;
            return result;
        } catch (err) {
            this.messageService.displayMessage('Failed loading client origin cors uri\'s');
  ...
  async onSearch(value: string): Promise<void> {
      ...
      if (value !== '')
      {
          this.dataSourceRequest.filter = new DataSourceRequestFilter("Origin.ToLower()", DataSourceOperator.Contains,value == '' ? null : value, DataSourceLogic.Or)
      }
      else
      {
  ...
  async export()
  {
      ...
      XLSX.writeFile(workBook, 'ClientCorsOriginUris.xlsx');
  }
  ...
  async removeCorsOrigins() {
    try {
      this.isLoadingResults = true;
      await this.service
        .removeCorsOriginUrisFromClient(this.selection.selected)
        .toPromise();
      this.messageService.displayMessage("Removed Client Origin Uri's");
      this.isLoadingResults = false;
      this.refresh();
    } catch (error) {
      this.messageService.displayMessage("Failed removing Client Origin Uri's");
      this.isLoadingResults = false;
    }
  }

  async removeCorsOrigin(clientCorsUri: ClientCorsUri) {
    try {
      this.isLoadingResults = true;
      const corsOrigins = new Array<ClientCorsUri>();
      corsOrigins.push(clientCorsUri);
      await this.service.removeCorsOriginUrisFromClient(corsOrigins).toPromise();
      this.messageService.displayMessage("Removed : " + clientCorsUri.origin);
      this.isLoadingResults = false;
      this.refresh();
    } catch (error) {
      this.messageService.displayMessage(
        "Failed removing Client Origin Uri's: " + clientCorsUri.origin
      );
      this.isLoadingResults = false;
    }
  }
